...
// Function for detecting what browser used
// if( OS != "windows" && OS != "macintosh" && OS != "linux" && OS != "solaris" )
...
//  If user use mobile browser the slider will not to work.
...
// --------------------------- //
	...
	// Slider and Window Coors 
		...
		// First is downloading a default background image
		...
		for( countSliderNum = 1; countSliderNum <= countSliders; countSliderNum++) {
		...
		// The first switch-button must be active
		...
		$(".slider-num").click(function() {
				...
				for( indexSlideNum = 0; indexSlideNum <= countSliders - 1; ++indexSlideNum) {
					...
					if( indexSlideNum == indexSlide ) {   // for needing button slide class is active
						...
						$(".slider-num:eq("+indexSlideNum+")").addClass("bg-slider-num-active");
						// other switch-button must be in the default state
						// if the no current switch button has class active is need to remove this active class
					} else if ( $(".slider-num:eq("+indexSlideNum+")").hasClass("bg-slider-num-active") ){ // for others class is default
						// remove the active class if it is no need
						$(".slider-num:eq("+indexSlideNum+")").removeClass("bg-slider-num-active");
